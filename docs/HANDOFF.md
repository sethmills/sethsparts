...
## Current live state (last refreshed 2026-09-15)
...
- **Version / releases:** `inventory/version.py` is `0.5.0`, tagged `v0.5.0` and pushed, with GitHub Releases cut from the tag annotations — **that is the changelog**, there is deliberately no CHANGELOG file. (`v0.4.0` has a tag but no release — a gap left from the v0.4.0 ship.) The update check asks `releases/latest` (falling back to `tags`), which only works once the repository is public: it answered **HTTP 404 for as long as the repo was private** (item 29). Verified working on prod — it reports "You're up to date (0.5.0)". Note GitHub picks "latest" by *release creation time*, not version: the older releases were created after v0.3.0 and GitHub labelled v0.2.0 as Latest, which would have made every install compare against the wrong version. Fixed with `gh release edit v0.3.0 --latest`; v0.5.0 was created with `--latest`.
- **Secrets:** live only in `/opt/sethsparts/.env` on the server (gitignored, never committed) — see `.env.example` in the repo root for the full list with explanations: `DJANGO_SECRET_KEY` and `DJANGO_ALLOWED_HOSTS` (both required), `TUNNEL_TOKEN` (required *on this server*, because it runs the tunnel profile), plus optional `VOICE_SEARCH_API_KEY`, `LED_CONTROLLER_URL`/`KEY`, `LABEL_PRINTER_URL`/`KEY`, `KIOSK_AUTOLOGIN_TOKEN`/`USERNAME`, `FEEDBACK_KEY` (the feedback relay's shared secret).
...
### 32. Community search, messaging, and email notifications — ✅ built (2026-09-14)

Three features, all riding the existing peer-connection model, each committed and tested.

**Community search** (`/community/search/`, "Search friends" in nav; inbound `GET /api/community/peer-search/`). The two halves of the spec: a peer with `shares_parts` may search the categories an owner marked shareable, and the page fans out to every such peer. The privacy boundaries are the point — the inbound query is filtered to `category__is_shareable=True`, quantities are fuzzed to "some"/"a few"/"none" (never a count, never a drawer/bin), and every search is written to `PeerSearchLog` so the owner can check it. Kept on a separate page deliberately, so local results never mix with peers'.

**Messaging** (`/community/messages/`; inbound `POST /api/community/messages/`). One-to-one text messages between connections; connecting is the consent, so there is no separate message permission. Delivery is point-to-point over the per-peer key, "attempt now, retry by hand" — no background queue. `remote_id` makes a retry idempotent, so a lost response cannot duplicate a message.

**Blocking.** `Peer.blocked` is a *sticky* form of revoke: block cuts messaging, search, and pins at once (all three already gate on `status=ACTIVE`), and — unlike a plain disconnect — a blocked workshop is refused by `claim()` even with a fresh pairing code, until explicitly unblocked. The one hole in "disconnect" that a harasser could exploit.

**Email notifications** (`/setup/email/`, Settings only, not a wizard step). SMTP (a Gmail app password is the documented, click-by-click path; any provider works), a master switch plus per-event toggles for message/connection/search, and a "send test email" button. Sending is synchronous and best-effort: a failed send never breaks the inbound request that triggered it. Nothing is sent until it is configured *and* enabled, so the default path touches no SMTP at all.

**Docs:** README's "Community search (specified, not built)" section replaced with a "Community: search, messages, and email" section plus feature-tour entries. A sweep of the README and the two `PLAN_community_*.md` docs removed verbatim quotes of the owner and "in the owner's words" phrasing (the repo is public); the app name was kept as branding, the owner's name stays in `HANDOFF.md`/`NEXT_SESSION.md` (internal logs, at the owner's request), and the personal copyright line was removed from the README and LICENSE.

**Verified:** 898 tests pass (69 new), 88/88 mutations caught, migrations `0026`/`0027` exercised forward/back/forward against a populated DB. **Not pushed or deployed** (standing gate).

### 33. Intake locations, in-app enrichment + web research, one-click upgrade — ✅ built & shipped (2026-09-15)

One batch closing out every item Seth listed for the "finished place" milestone, plus the web-research follow-up. Shipped as `v0.4.0` (tag pushed; repo already public), deployed to prod, host upgrade-agent installed, Tavily key set and verified live.

**Intake / locations.** `IntakeNote` gained nullable `drawer` + `bin` FKs (migration 0031) and a `location` property resolving the leaf (bin → drawer → container). `/intake/add/` now has a **Location** picker — Drawer → Bin cascade (bins filter to the chosen drawer, plus box/tote) — instead of "Box (optional)". "＋ Add new location" implements Seth's fork exactly: *"will this location have bins inside it?"* → cabinet (a container) / bin-in-a-drawer / tote, each barcoded via generate-and-print or scan. `/intake/new-box/` is now one screen: create the tote, list contents (typed **or** dictated — dictation JS extracted to `_dictate.html`), print the barcode.

**Enrichment overhaul.** The export→Claude→import loop is replaced by in-app DeepSeek, with web research on top: Tavily searches (DeepSeek's chat API can't browse — its Responses API *ignores* `web_search`), DeepSeek reads the results and extracts category/description/manufacturer/product URL/datasheet URL/pinout URL/price/confidence. `/enrichment/` now has **Enrich with DeepSeek** (full research when both keys are set, descriptive-only fallback otherwise), **AI scan for candidates** (one batched DeepSeek call triaging names), **bulk classify** (inline status dropdowns, no click-through), and **flag for enrichment** on every part page. `inventory/research.py` is the new module; `ai_classify_candidates` added to `enrichment_ai.py`.

**AI keys, web-settable.** Keys were env-var-only; now also settable in Settings (migration 0033) via a **Research & AI** wizard/settings page with step-by-step instructions for DeepSeek and Tavily and a "Test both keys" button. `site_config.ai_api_key(env, field)` resolves Settings-first-then-env, so `@override_settings` tests still pass and Docker env vars remain the fallback.

**One-click upgrade.** Settings' "Upgrade now" writes `data/upgrade-request`; a host agent (`deploy/upgrade.sh` + `sethsparts-upgrade.service`/`.timer`, installed on Hetzner) backs up the DB, `git pull --ff-only`, rebuilds, restarts, writes `data/upgrade-result` back. No-suicide by construction: marker-gated, `--ff-only`, always writes a result. The web process never rebuilds its own container.

**Kiosk keyboard.** Re-added as opt-in: `SiteSettings.show_onscreen_keyboard` (migration 0032) shows a ⌨ button on the Pi kiosk, wired to `/toggle-keyboard` in the kiosk helper (toggles squeekboard); the fullscreen z-ordering caveat stays in the helper's docstring.
