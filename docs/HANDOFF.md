...
- **Test suite:** `./venv/bin/python manage.py test inventory` — 898 tests, ~45s, fully offline (external HTTP mocked, no hardware needed). Also `./venv/bin/python scripts/mutation_check.py` — 88 deliberate breakages, currently 88/88 caught; it has to stay at 100% before a deploy. See "Tests" in `README.md`, and item 21 below.
