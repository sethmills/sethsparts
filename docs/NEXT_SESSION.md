...
1. ~~**Flip the repo to public.**~~ **Done (2026-09-14)** — and it was worth doing in this order: the two inventory files that had been committed (`parts_review.xlsx`, `parts_review.md`, plus the clarification worklist) were purged from history with `git filter-repo` *first*, so they were never reachable in the public repository at any point. GitHub Releases now exist for `v0.1.0`, `v0.2.0` and `v0.3.0`, and the update check works for the first time (it 404'd while the repo was private). See HANDOFF item 31. Still open from that list: whether you want issues and PRs enabled at all.
