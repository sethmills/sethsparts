...
- **Guided install for a clone deployment (your dad's).** Explicitly deferred — "not at this moment... when we are finished." Eventually: clone this repo for someone else's workshop (different LED array, possibly a different label printer, same drawer/row/bin structure), with a guided setup covering rebranding and flashing the Scorpio, and the big architectural difference — running fully locally on that person's own Pi rather than on an externally-hosted box like your Hetzner setup. Revisit once yours feels "complete."
